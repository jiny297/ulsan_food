...
function openMonthlyRecommend() {
    ...
    list.innerHTML = sorted.map(r => `
        <p>🍽️ <strong>${r.name}</strong> (${r.rating}⭐)</p>
    `).join('');
...
function openCategorySearch(category) {
    ...
    title.textContent = `🍽️ ${category} 맛집`;
    container.innerHTML = '';
...
function openMyReviews() {
    ...
    if (myReviews.length === 0) {
        ...
        list.innerHTML = myReviews.map((review, index) => `
            <div style="
                background: #f8f9fa;
                padding: 20px;
                border-radius: 10px;
            ...
            ">
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px;">
                    <strong style="color: #667eea;">${review.restaurantName}</strong>
...
function openPointHistory() {
    ...
    list.innerHTML = `
        <div style="
            background: linear-gradient(135deg, #95cfcf 0%, #3C8F8F 100%);
            color: white;
            padding: 30px;
            border-radius: 15px;
            text-align: center;
    ...
    if (pointHistory.length === 0) {
        list.innerHTML += `
            <div style="text-align: center; padding: 40px; color: #777;">
                포인트 내역이 없습니다 🏅<br>
                <span style="font-size:13px; color:#aaa;">
                    맛집 추천, 리뷰 작성으로 포인트를 적립하세요!
                </span>
            </div>
        `;
    } else {
        list.innerHTML += pointHistory.map(history => `
            <div style="
                background: #f8f9fa;
                padding: 15px 20px;
                border-radius: 10px;
            ...
            ">
                ...
                <div style="
                    font-weight: bold;
                    font-size: 1.2rem;
                    color: ${history.type === 'earn' ? '#51cf66' : '#ff6b6b'};
                ">
